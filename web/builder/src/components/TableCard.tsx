...
import type { Table } from "@dux/core";
import { isDateType } from "@dux/core";
...
function CalendarIcon({ size }: { size?: number }) {
  return (
    <svg width={size ?? 13} height={size ?? 13} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
export default function TableCard(props: {
  ...
  const columns = Object.values(props.table.Columns)
  ...
  return (
    <div
      className={`${styles.card}${isHidden ? ` ${styles.cardHiddenState}` : ""}`}
      data-card={props.tableName}
      style={{ left: props.x, top: props.y }}
    >
      <div className={styles.cardHeader} onMouseDown={props.onHeaderMouseDown}>
        <span className={styles.cardTableName} title={props.tableName}>
          {props.tableName}
        </span>
        {props.table.IsView && (
        ...
        {dateColumns.length > 0 && (
          <button
            className={`${styles.iconBtn}${isDateTable ? ` ${styles.dateBtnActive}` : ""}`}
            title={
              isDateTable
        ...
        <button
          className={`${styles.iconBtn}${isHidden ? ` ${styles.hiddenBtnActive}` : ""}`}
      ...
      <div className={styles.cardColumns} data-card-columns onScroll={() => props.onColumnsScroll?.()}>
        {columns.map((col) => (
          <div
            key={col.Name}
            className={`${styles.colRow}${col.Hidden === true ? ` ${styles.colRowHidden}` : ""}`}
            data-table={props.tableName}
            data-col={col.Name}
          >
            <TypeIcon dataType={col.DataType} />
            <span className={styles.colName} title={col.Name}>
              {col.Name}
            </span>
            {/* When the designated date table has several date columns, each
                shows a calendar toggle to pick which one is THE date column. */}
            {isDateTable && dateColumns.length > 1 && isDateType(col.DataType) && (
              <button
                className={`${styles.colCalBtn}${props.dateColumn === col.Name ? ` ${styles.colCalBtnActive}` : ""}`}
                title={
            ...
            <button
              className={`${styles.colHideBtn}${col.Hidden === true ? ` ${styles.colHideBtnActive}` : ""}`}
