...
db/                  Data and metadata databases
  ...
  *.duckdb / *.db    Your data files — attached read-only by duxd
...
| `--import` | — | Import a `dux.toml` into the sidecar metadata DB and exit |

## Server (`duxd`)

Place your data files in `db/`. `duxd` opens `db/dux.duckdb` as a read-write metadata store and attaches every other `*.duckdb` / `*.db` file in that directory as a read-only named attachment.
