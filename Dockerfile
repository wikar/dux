...
EXPOSE 8080
# ENTRYPOINT, not CMD, so `docker run <image> --max-connections 5` appends flags
# to duxd instead of replacing the command. The image also ships the dux CLI;
# reach it with `--entrypoint ./dux`.
ENTRYPOINT ["./duxd"]
